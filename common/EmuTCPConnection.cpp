...
bool EmuTCPConnection::LineOutQueuePush(char* line) {
	...
	if(line[0] == '*') {
		...
		if (strcmp(line, "**PACKETMODEGS**") == 0) {
			MSendQueue.lock();
			safe_delete_array(sendbuf);
			if (TCPMode == modeConsole)
				Send((const uchar*) "\0**PACKETMODEGS**\r", 18);
			TCPMode = modePacket;
			PacketMode = packetModeGroupServ;
			EmuTCPNetPacket_Struct* tnps = 0;
			while ((tnps = InModeQueue.pop())) {
				SendPacket(tnps);
				safe_delete_array(tnps);
...
bool EmuTCPConnection::ConnectIP(uint32 irIP, uint16 irPort, char* errbuf) {
	...
	MSendQueue.lock();
		...
		if (pOldFormat) {
			...
			} else if(PacketMode == packetModeGroupServ) {
				safe_delete_array(sendbuf);
				sendbuf_size = 18;
				sendbuf_used = sendbuf_size;
				sendbuf = new uchar[sendbuf_size];
				memcpy(sendbuf, "\0**PACKETMODEGS**\r", sendbuf_size);
			} else {
